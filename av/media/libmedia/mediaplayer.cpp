...
static bool setParameterForAudioST(const sp<IMediaPlayer>& player)   //For MP3 SeekTable
{
    ...
    if(NULL != f)
        fclose(f);
    if((!strncasecmp(process_name, "com.android.music", 17)) || (!strncasecmp(process_name, "com.google.android.music", 24))) {
        Parcel request ;
        ...
        if (p != NULL) {
            err = p->setParameter(KEY_PARAMETER_PLAYBACK_MTK, requestPlayback);
        }
        ALOGD("%s is mtk playback %d",process_name,err);
    } else if (!strncasecmp(process_name, "com.android.gallery3d", 21)) {
        Parcel request ;
        status_t err = NO_ERROR;
        request.writeInt32(1);
        if (p != NULL) {
            err = p->setParameter(KEY_PARAMETER_PLAYBACK_MTK, request);
        }
        ALOGD("%s is mtk playback %d",process_name,err);
    }
    else {
...
#ifdef MTK_DRM_APP
static bool checkDrmPermission(int fd,const char *url, const sp<IMediaPlayer>& player) {
    ALOGD("setDataSource_drm_preCheck: fd %d url %p", fd, url);
    bool isOMADrm = false;
    String8 drmProcName;
    drmProcName  = DrmMtkUtil::getProcessName(getpid()); // current client process
    ALOGD("setDataSource_drm_preCheck: fd %d url %p, %s", fd, url, drmProcName.string());
    if (url == NULL) {
        // file descriptor case, check if it's a DCF file
        // and check if it's a trusted audio / video playback client for drm
        isOMADrm = DrmMtkUtil::isDcf(fd);
        if (isOMADrm) {
            drmProcName  = DrmMtkUtil::getProcessName(getpid()); // current client process
            if (!DrmMtkUtil::isTrustedClient(drmProcName)) {
                ALOGW("setDataSource with fd: untrusted client [%d][%s], denied to access drm fd [%d]",
                        getpid(), drmProcName.string(), fd);
                return false;
            }
    ...
    ALOGD("save process info: [%s]", drmProcName.string());
    Parcel request;
    request.writeString8(drmProcName);
    request.setDataPosition(0);
    player->setParameter(KEY_PARAMETER_DRM_CLIENT_PROC, request);
    return isOMADrm;
}
#endif
#endif
...
#ifdef MTK_DRM_APP
            if (err == NO_ERROR) {
                checkDrmPermission(0, url, player);
            }
#endif
        }
    }
...
#ifdef MTK_DRM_APP
        if (err == NO_ERROR) {
            checkDrmPermission(fd, NULL, player);
        }
#endif
    }
    return err;
...
#ifdef MTK_AOSP_ENHANCEMENT
        if(durationMs < 0){
           SLOGD("getDuration:%d < 0 ",durationMs);
           durationMs = 0;
        }
#endif
        if (ret != OK) {
