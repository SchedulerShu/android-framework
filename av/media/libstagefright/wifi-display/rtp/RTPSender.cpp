...
void RTPSender::queuePackets_pro(
        const sp<ABuffer> &Packets, int64_t ,int64_t ,int64_t /*startSendUs*/,int64_t /*delayUs*/){
...
//    bool  isVideo = isVideoBuffer(tsPackets);
    ...
    if(!tsPackets->meta()->findInt64("latencyB", &latencyB)){
        latencyB = -1;
    }
    if(!tsPackets->meta()->findInt64("LatencyToken", &LatencyToken)){
        LatencyToken = -1;
    }
