...
void AudioFlinger::EffectModule::process()
{
    ...
    if (isProcessEnabled()) {
        ...
        AudioDump::dump(fileName, mConfig.inputCfg.buffer.s16, mConfig.inputCfg.buffer.frameCount * 2 * 2,
            gaf_effect_propty);
#endif
#endif
        ...
        AudioDump::dump(fileName, mConfig.outputCfg.buffer.s16, mConfig.outputCfg.buffer.frameCount * 2 * 2,
            gaf_effect_propty);
#endif
#endif
...
#ifdef MTK_HIFI_AUDIO
    if (OUTPUT_RATE_192 == thread->sampleRate() || OUTPUT_RATE_96 == thread->sampleRate() ||
        OUTPUT_RATE_176_4 == thread->sampleRate() || OUTPUT_RATE_88_2 == thread->sampleRate()) {
        ...
        ALOGD("effect configure framecount %zu", framecount);
...
void AudioFlinger::EffectChain::setEffectSuspendedAll_l(bool suspend)
{
    ...
    if (suspend) {
        ...
        if (--desc->mRefCount == 0) {
            ...
            ALOGD("setEffectSuspendedAll_l, try to remove index = %zu", index);
            ssize_t newIndex = mSuspendedEffects.indexOfKey((int)kKeyForSuspendAll);
            ALOGD("setEffectSuspendedAll_l, update index = %zu before delete", newIndex);
            if (newIndex >= 0) {
                ALOGV("setEffectSuspendedAll_l() remove entry for %08x",mSuspendedEffects.keyAt(newIndex));
                mSuspendedEffects.removeItem((int)kKeyForSuspendAll);
            } else {
                ALOGD("setEffectSuspendedAll_l index = %zu, someone already delete the index", newIndex);
            }
#else
