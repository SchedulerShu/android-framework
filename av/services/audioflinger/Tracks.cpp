...
void AudioFlinger::ThreadBase::TrackBase::releaseBuffer(AudioBufferProvider::Buffer* buffer)
{
    ...
    if (!mIsOut) {
        const int SIZE = 256;
        char fileName[SIZE];
        sprintf(fileName, "%s_%p_%p_in_%d.pcm", gaf_track_pcm, this, mServerProxy, mTrackCount);
        ...
        AudioBufferProvider::Buffer* buffer)
{
    ...
    if (NULL == buffer->raw) {
        ...
        if (mIsOut) {
            const int SIZE = 256;
            char fileName[SIZE];
            sprintf(fileName, "%s_%p_%p_out_%d.pcm", gaf_track_pcm, this, mServerProxy, mTrackCount);
            //AudioDump::dump(fileName,buffer->raw,buffer->frameCount*mFrameSize,gaf_track_propty);
            AudioDump::threadDump(fileName, buffer->raw, buffer->frameCount * mFrameSize, gaf_track_propty,
                    mFormat, mSampleRate, mChannelCount);
        }
...
bool AudioFlinger::PlaybackThread::Track::isReady() const {
    ...
    if (framesReady() >= mServerProxy->getBufferSizeInFrames() ||
        ...
        mFillingUpStatus = FS_FILLED;
        android_atomic_and(~CBLK_FORCEREADY, &mCblk->mFlags);
        ALOGD_IF(!isReady, "%s_3 => true, track(%p): mState %d, mFillingUpStatus %d, mIsPausedToFlush %d",
            __FUNCTION__, this, mState, mFillingUpStatus, mIsPausedToFlush);
        isReady = true;
        return true;
    }
    ALOGD_IF(isReady, "%s_4 => false, track(%p): mState %d, mFillingUpStatus %d, mIsPausedToFlush %d",
        __FUNCTION__, this, mState, mFillingUpStatus, mIsPausedToFlush);
    isReady = false;
    return false;
}
