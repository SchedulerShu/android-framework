    ...
    */
    ...
    private static int applyLocalCallCapabilities(ImsCallProfile localProfile, int capabilities) {
        ...
        switch (localProfile.mCallType) {
            case ImsCallProfile.CALL_TYPE_VT:
                /// M: vilte call always can downgrade to voice, so add downgrade capability. @{
    ...
    public boolean update(ImsCall imsCall, ImsPhoneCall.State state) {
        if (state == ImsPhoneCall.State.ACTIVE) {
            ...
            /// M: ALPS02617050.
            /// Sometimes swap is triggered from lower layer. e.q. merge conference. @{
        ...
        if (callProfile != null) {
            ...
            if (Phone.DEBUG_PHONE) {
                Rlog.d(LOG_TAG, "address = " +  address + " name = " + name +
                        " nump = " + nump + " namep = " + namep + " mAddr = " + mAddress);
            }
            /// M: ALPS02800458. Fix google issue, address should be updated. @{
            //if(equalsHandlesNulls(mAddress, address)) {
            if(!equalsHandlesNulls(mAddress, address)) {
                Rlog.d(LOG_TAG, "update address = " + address);
            /// @}
                mAddress = address;
                changed = true;
            }
