...
public class DataConnection extends StateMachine {
    ...
    static final int EVENT_IPV4_ADDRESS_REMOVED = BASE + 16;
    static final int EVENT_IPV6_ADDRESS_REMOVED = BASE + 17;
    static final int EVENT_ADDRESS_REMOVED = BASE + 18;
    static final int EVENT_VOICE_CALL = BASE + 19;
    static final int EVENT_FALLBACK_RETRY_CONNECTION = BASE + 20;

    private static final int CMD_TO_STRING_COUNT =
            EVENT_FALLBACK_RETRY_CONNECTION - BASE + 1;
    ...
    /// M: This method will not be called, so don't add logic here.
    void dispose() {
    ...
    public boolean isIpv4Connected() {
        ...
        for (InetAddress addr: addresses) {
            if (DBG) log("isIpv4Connected(), addr:" + addr);
    ...
    private void tearDownData(Object o) {
        ...
        if ((o != null) && (o instanceof DisconnectParams)) {
            ...
            } else if (TextUtils.equals(dp.mReason, Phone.REASON_RA_FAILED)) {
                if (mValid == RA_LIFE_TIME_EXPIRED) {
                    discReason = RILConstants.DEACTIVATE_REASON_NORMAL;
                } else if (mValid == RA_INITIAL_FAIL) {
                    discReason = RILConstants.DEACTIVATE_REASON_RA_INITIAL_FAIL;
                } else if (mValid == RA_REFRESH_FAIL) {
                    discReason = RILConstants.DEACTIVATE_REASON_RA_REFRESH_FAIL;
                }
            } else if (TextUtils.equals(dp.mReason, Phone.REASON_PCSCF_ADDRESS_FAILED)) {
                /// M: P-CSCF error handling.
                discReason = RILConstants.DEACTIVATE_REASON_NO_PCSCF;
            }
        }
    ...
    private NetworkCapabilities makeNetworkCapabilities() {
        ...
        boolean isDataEnabled = mDct.getDataEnabled();
            ...
            for (String type : mApnSetting.types) {
                switch (type) {
                    case PhoneConstants.APN_TYPE_ALL: {
                        if (isDataEnabled && isDefaultDataSubPhone(mPhone)) {
    ...
    private class DcDefaultState extends State {
        ...
        public void enter() {
            ...
            // M: register network observer for RA fail
        ...
        public void exit() {
            ...
            // M: unregister network observer for RA fail
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case DcAsyncChannel.REQ_GET_APNTYPE: {
                    ...
                    if (VDBG) {
                        log("REQ_GET_APNTYPE  aryApnType=" + aryApnType);
                    }
                    mAc.replyToMessage(msg, DcAsyncChannel.RSP_GET_APNTYPE, aryApnType);
                    break;
                }
                        ...
                        case ERR_RilError:
                            ...
                            if (result.mFailCause == DcFailCause.PDP_FAIL_FALLBACK_RETRY) {
                                ...
                                if (mDcFcMgr != null && mDcFcMgr.isSpecificNetworkAndSimOperator(
                                    ...
                                    deferMessage(obtainMessage(EVENT_FALLBACK_RETRY_CONNECTION,
                                            mTag));
                                }
                                transitionTo(mActiveState);
                            } else {
    ...
    private class DcActiveState extends State {
        @Override public void enter() {
            ...
            /** M: if this DC is used by two different ApnContext, when just one disconnect,
             *  we still need create DcNetworkAgent for another one.
             */
            if (((hasMessages(EVENT_DISCONNECT) || hasDeferredMessages(EVENT_DISCONNECT))
                    && mApnContexts.size() == 1) ||
        ...
        public boolean processMessage(Message msg) {
            ...
            switch (msg.what) {
                case EVENT_CONNECT: {
                    ...
                    // M: update capabilities, default capbility maybe need update.
                    if (mNetworkAgent != null) {
                ...
                case EVENT_IPV6_ADDRESS_REMOVED:
                    ...
                    if (mInterfaceName != null && mInterfaceName.equals(addrV6Info.mIntfName)) {
                        if (!BSP_PACKAGE) {
                            try {
                            mValid = mGsmDCTExt.getIPv6Valid(addrV6Info.mLinkAddr);
                            ...
                            if (mValid == RA_LIFE_TIME_EXPIRED ||
                                ...
                                log("DcActiveState: RA is failed or life time expired,"
                                        + " valid:" + mValid);
                                onAddressRemoved();
                            }
                        }
                ...
                case EVENT_SETUP_DATA_CONNECTION_DONE: {
                    ...
                    switch (result) {
                        case SUCCESS:
                            ...
                            break;
                        case ERR_RilError:
                            String str = "DcActiveState_FALLBACK_Retry: ERR_RilError "
                                    + " result=" + result
                                    + " result.isRestartRadioFail=" +
                                    result.mFailCause.isRestartRadioFail()
                                    + " result.isPermanentFail=" +
                ...
                for (ConnectionParams cp : mApnContexts.values()) {
                    ...
                    if (apnContext.getState() == DctConstants.State.CONNECTED) {
                        if (isApnTypeImsOrEmergency(apnType)) {
                            ...
                            if (DBG) {
                                log("apnType: " + apnType
                                        + ", skip disconnect while onAddressRemoved!!");
                            }
                        } else {
    ...
    // M: Customize fallback retry start
    /**
    ...
    // M: Customize fallback retry end
    ...
    public void handlePcscfErrorCause(DisconnectParams dp) {
        ...
        do {
            ...
            boolean syncFailCause = b.getBoolean(
                    CarrierConfigManager.KEY_IMS_PDN_SYNC_FAIL_CAUSE_TO_MODEM_BOOL);

            log("handlePcscfErrorCause() syncFailCause: " + syncFailCause +
                    " mPcscfAddr: " + mPcscfAddr +
                    " with subId: " + subId);
            ...
            if (syncFailCause) {
                ...
                if (TextUtils.equals(apnType, PhoneConstants.APN_TYPE_IMS) &&
                    ...
                    dp.mReason = Phone.REASON_PCSCF_ADDRESS_FAILED;
