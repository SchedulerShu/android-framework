...
public abstract class Phone extends Handler implements PhoneInternalInterface {
    ...
    protected static final int EVENT_SPEECH_CODEC_INFO              = EVENT_MTK_BASE + 1;
    /// M: CC: Proprietary incoming call handling
    protected static final int EVENT_VOICE_CALL_INCOMING_INDICATION = EVENT_MTK_BASE + 2;
    ...
    public void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            ...
            case EVENT_SET_NETWORK_MANUAL_COMPLETE:
                // M ADD; if md report error, it is auto mode now;
                // clear setting to avoid the notification.
        ...
        switch(msg.what) {
            ...
            case EVENT_CHARGING_STOP:
                ...
                sFun[0] = "AT+EFUN=1";
                sFun[1] = "";
                this.invokeOemRilRequestStrings(sFun, null);

                //sRat[0] = "AT+ERAT=3";
                //sRat[1] = "";
                //this.invokeOemRilRequestStrings(sRat, null);

                s[0] = "AT+ERFTX=1,0,0," +
                        SystemProperties.get("persist.radio.charging_stop", "40");
                s[1] = "";
    ...
    protected void notifyServiceStateChangedP(ServiceState ss) {
        ...
        if ((ss.getState() != ServiceState.STATE_POWER_OFF) &&
                (SystemProperties.get("ril.charging_stop_enable", "0").equals("1"))) {
    ...
    */
    ...
    public void setPreferredNetworkType(int networkType, Message response) {
        /// M: [CDMA] @{
        if (getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA
                || getPhoneType() == PhoneConstants.PHONE_TYPE_CDMA_LTE) {
    ...
    public void radioCapabilityUpdated(RadioCapability rc) {
        ...
        if (SubscriptionManager.isValidSubscriptionId(getSubId())) {
            // MADD, check restore Nw selection based on skip_restoring_network_selection
            boolean skipRestoringSelection = mContext.getResources().getBoolean(
    ...
    /**
        * Returns current located PLMN string(ex: "46000") or null
        * if not availble (ex: in flight mode or no signal area)
        * @internal
        */
    ...
    public void triggerModeSwitchByEcc(int mode, Message response) {
        mCi.triggerModeSwitchByEcc(mode, response);
    }
