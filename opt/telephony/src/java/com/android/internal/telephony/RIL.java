...
public final class RIL extends BaseCommands implements CommandsInterface {
    ...
    private final BroadcastReceiver mAirplaneModeListener = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            boolean bAirplaneModeOn = intent.getBooleanExtra("state", false);

            Rlog.d(RILJ_LOG_TAG, "ACTION_AIRPLANE_MODE_CHANGED, bAirplaneModeOn = " +
                   bAirplaneModeOn);
            if (bAirplaneModeOn) {
                Rlog.d(RILJ_LOG_TAG, "Clean CFU status.");
    ...
    public RIL(Context context, int preferredNetworkType,
        ...
        if (cm.isNetworkSupported(ConnectivityManager.TYPE_MOBILE) == false) {
            ...
            // modify for phone init too late
            mDefaultDisplayState = mDefaultDisplay.getState();
            ...
            context.registerReceiver(mAirplaneModeListener, filterAirplane);
        }
    ...
    public void setPsRegistration(boolean register, int mode, Message result) {
        // Note: restricted: true (attach) / false (detach)
        //          mode : 0 (when needed/blocked) / 1 (always)
        //          if attach, mode will be auto-modifed to 0 (unblocked).
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_PS_REGISTRATION, result);
        if (RILJ_LOGD) {
            riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) +
    ...
    public void writeSmsToRuim(int status, String pdu, Message response) {
        ...
        rr.mParcel.writeInt(status);
        // MTK-START: in ril.cpp, dispatchRilCdmaSmsWriteArgs excepts RIL_CDMA_SMS_Message
        // when doing parcel to data structure conversion.
        // Keep the AOSP design in ril.cpp and change the AOSP code here.
        /*
        rr.mParcel.writeString(pdu);
        */
    ...
    setupDataCall(int radioTechnology, int profile, String apn,
        ...
        int interfaceId = 1;
        setupDataCall(radioTechnology, profile, apn, user, password, authType, protocol,
                interfaceId, result);
    }
    ...
    setupDataCall(int radioTechnology, int profile, String apn,
            String user, String password, int authType, String protocol,
            int interfaceId, Message result) {
        RILRequest rr
    ...
    onRadioAvailable() {
        ...
        updateScreenState();
        /// M: force update the current screen state
        sendScreenState(mRadioScreenState == RADIO_SCREEN_ON);
   }
    ...
    processSolicited (Parcel p, int type) {
        ...
        if (rr.mRequest == RIL_REQUEST_SET_PREFERRED_NETWORK_TYPE) {
            if ((error != 0) && (mPreviousPreferredType != -1)) {
                riljLog("restore mPreferredNetworkType from "
                        + mPreferredNetworkType + " to " + mPreviousPreferredType);
 ...
 | sed -re 's/\{([^,]+),[^,]+,([^}]+).+/case \1: ret = \2(p); break;/'
            ...
            case RIL_REQUEST_QUERY_UPB_AVAILABLE : ret = responseInts(p); break;
            case RIL_REQUEST_READ_EMAIL_ENTRY : ret = responseString(p); break;
            case RIL_REQUEST_READ_SNE_ENTRY: ret = responseString(p); break;
            case RIL_REQUEST_READ_ANR_ENTRY: ret = responsePhbEntries(p); break;
            case RIL_REQUEST_READ_UPB_AAS_LIST: ret = responseStrings(p); break;
            // PHB End
            ...
            // M: Fast Dormancy
            case RIL_REQUEST_SET_FD_MODE: ret = responseVoid(p); break;
            ...
            // M: CC33 LTE End.
            // M: For OP12 Start.
            case RIL_REQUEST_SYNC_APN_TABLE: ret = responseVoid(p); break;
            // M: For OP12 End.
            ...
            /// M: IMS ViLTE feature. @{
            case RIL_REQUEST_VT_DIAL_WITH_SIP_URI: ret = responseVoid(p); break;
            /// @}

            case RIL_REQUEST_SWITCH_MODE_FOR_ECC: ret = responseVoid(p); break;
            case RIL_REQUEST_SET_PS_REGISTRATION: ret = responseVoid(p); break;
            case RIL_REQUEST_SYNC_DATA_SETTINGS_TO_MD: ret = responseVoid(p); break;
            // IR and WorldPhone
...
*/
            ...
            case RIL_UNSOL_CALL_REDIAL_STATE: ret = responseInts(p); break;
            ...
            // M: [LTE][Low Power][UL traffic shaping] End
            case RIL_UNSOL_MODULATION_INFO: ret = responseInts(p); break;
            case RIL_UNSOL_NETWORK_EVENT: ret = responseInts(p); break;
            /// M: For CDMA call accepted @{
            case RIL_UNSOL_CDMA_CALL_ACCEPTED: ret = responseVoid(p); break;
            /* M: eMBMS feature */
            case RIL_UNSOL_EMBMS_SESSION_STATUS: ret = responseInts(p); break;
            /// @}
            case RIL_UNSOL_PCO_STATUS: ret = responseInts(p); break;
            /// M: IA-change attach APN
            case RIL_UNSOL_DATA_ATTACH_APN_CHANGED: ret = responseInts(p); break;
            // MTK Start
            case RIL_UNSOL_TRIGGER_OTASP: ret = responseVoid(p); break;
            // MTK End
        ...
        switch(response) {
            ...
            case RIL_UNSOL_VOICE_RADIO_TECH_CHANGED:
                ...
                /// M: Save new Voice Radio technology
                mNewVoiceTech[0] = ((int[]) ret)[0];
            ...
            case RIL_UNSOL_INCOMING_CALL_INDICATION:
                ...
                if (mIncomingCallIndicationRegistrant != null) {
                    mIncomingCallIndicationRegistrant.notifyRegistrant(
                            new AsyncResult(null, ret, null));
                }
                break;
            /// @}
            ...
            case RIL_UNSOL_CIPHER_INDICATION:
                ...
                riljLog("RIL_UNSOL_CIPHER_INDICATION :" + simCipherStatus +
                        " " + sessionStatus + " " + csStatus + " " + psStatus);
            ...
            /// M: CC: Vzw ECC/hVoLTE redial @{
            case RIL_UNSOL_CALL_REDIAL_STATE:
                if (RILJ_LOGD) unsljLogvRet(response, ret);
                if (mCallRedialStateRegistrants != null) {
                    int redialState = ((int[])ret)[0];
                    mCallRedialStateRegistrants.notifyRegistrants(
                            new AsyncResult(null, redialState, null));
                }
                break;
            /// @}
                        ...
                        if (!SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
                            try {
                                if (mServiceStateExt.needBrodcastAcmt(error_type, error_cause)
                                    ...
                                    Intent intent = new Intent(
                                            TelephonyIntents.
                                            ACTION_ACMT_NETWORK_SERVICE_STATUS_INDICATOR);
            ...
            case RIL_UNSOL_SIM_PLUG_OUT:
                ...
                int retCfValue[] = new int[2];
                retCfValue[0] = 0; // disabled
                retCfValue[1] = 1; // line 1

                riljLog("Notify CFU change to disable due to sim plug out.");
                mCallForwardingInfoRegistrants.notifyRegistrants(
                        new AsyncResult(null, retCfValue, null));
                break;
            ...
            case RIL_UNSOL_SIM_COMMON_SLOT_NO_CHANGED:
                ...
                if (mCommonSlotNoChangedRegistrants != null) {
                    mCommonSlotNoChangedRegistrants
                    .notifyRegistrants(new AsyncResult(null, null, null));
                }
                break;
            ...
            case RIL_UNSOL_IMS_REGISTRATION_INFO:
                ...
                if (mImsRegistrationInfoRegistrants != null) {
                    mImsRegistrationInfoRegistrants
                    .notifyRegistrants(new AsyncResult(null, ret, null));
                }
                break;
            ...
            case RIL_UNSOL_VOLTE_EPS_NETWORK_FEATURE_INFO:
                ...
                if (mEpsNetworkFeatureInfoRegistrants != null) {
                   mEpsNetworkFeatureInfoRegistrants
                   .notifyRegistrants(new AsyncResult(null, ret, null));
                }
                break;
            ...
            case RIL_UNSOL_SRVCC_HANDOVER_INFO_INDICATION:
                ...
                if (mSrvccHandoverInfoIndicationRegistrants != null) {
                    mSrvccHandoverInfoIndicationRegistrants
                    .notifyRegistrants(new AsyncResult(null, ret, null));
                }
                break;
            ...
            case RIL_UNSOL_EMERGENCY_BEARER_SUPPORT_NOTIFY:
                ...
                if (mEmergencyBearerSupportInfoRegistrants != null) {
                    mEmergencyBearerSupportInfoRegistrants
                    .notifyRegistrants(new AsyncResult(null, ret, null));
                }
                break;
            /// @}
            ...
            case RIL_UNSOL_MODULATION_INFO:
                if (RILJ_LOGD) unsljLogRet(response, ret);
                if (mModulationRegistrants != null) {
                    mModulationRegistrants.notifyRegistrants(new AsyncResult(null, ret, null));
                }
                break;
            case RIL_UNSOL_NETWORK_EVENT:
                if (RILJ_LOGD) unsljLogRet(response, ret);
                if (mNetworkEventRegistrants != null) {
                    mNetworkEventRegistrants.notifyRegistrants(
                            new AsyncResult(null, ret, null));
                }
                break;
            /// M: For CDMA call accepted @{
            ...
            case RIL_UNSOL_EMBMS_SESSION_STATUS:
                ...
                if (ret != null) {
                    ...
                    intent.putExtra(TelephonyIntents.EXTRA_IS_ACTIVE, status[0]);
                    mContext.sendBroadcast(intent);
                }
                break;
            case RIL_UNSOL_PCO_STATUS:
                if (RILJ_LOGD) {
                    unsljLogRet(response, ret);
                }
                if (mPcoStatusRegistrant != null) {
                    mPcoStatusRegistrant.notifyRegistrants(
                            new AsyncResult(null, ret, null));
                }
                break;
            // M: IA-change attach APN
            case RIL_UNSOL_DATA_ATTACH_APN_CHANGED:
                ...
                mAttachApnChangedRegistrants
                        .notifyRegistrants(new AsyncResult(null, ret, null));
                break;
            /// M: For CDMA call accepted @{
            case RIL_UNSOL_TRIGGER_OTASP:
                if (RILJ_LOGD) unsljLog(response);
                // OTASP
    ...
    responseICC_IOBase64(Parcel p) {
        ...
        return new IccIoResult(sw1, sw2,
    ...
    responseSetupDataCall(Parcel p) {
        ...
        if (ver < 5) {
            ...
            if (num != 1) {
                throw new RuntimeException(
                        "RIL_REQUEST_SETUP_DATA_CALL resp. expecting 1 RIL_Data_Call_response_v5"
                        + " got " + num);
            }
    ...
    responseOperatorInfosWithAct(Parcel p) {
        ...
        for (int i = 0 ; i < strings.length ; i += 5) {
            ...
            if (strings[i + 2] != null) {
                strings[i + 0] = SpnOverride.getInstance().lookupOperatorName(
                        SubscriptionManager
                        .getSubIdUsingPhoneId(mInstanceId), strings[i + 2], true, mContext);
                strings[i + 1] = SpnOverride.getInstance().lookupOperatorName(
                        SubscriptionManager
                        .getSubIdUsingPhoneId(mInstanceId), strings[i + 2], false, mContext);
                riljLog("lookup RIL responseOperator(), longAlpha= " + strings[i + 0]
                        + ",shortAlpha= " + strings[i + 1]
                        + ",numeric=" + strings[i + 2]);
            }
            ...
            longName = lookupOperatorNameFromNetwork(
                    SubscriptionManager.getSubIdUsingPhoneId(mInstanceId), strings[i + 2], true);
            shortName = lookupOperatorNameFromNetwork(
                    SubscriptionManager.getSubIdUsingPhoneId(mInstanceId), strings[i + 2], false);
            ...
            ///M: Not to show MVNO name for registered operator name display for certain SIM. @{
            if (!SystemProperties.get("ro.mtk_bsp_package").equals("1")) {
                try {
                    strings[i + 0] = mServiceStateExt.updateOpAlphaLongForHK(
                            strings[i + 0], strings[i + 2], mInstanceId);
                } catch (RuntimeException e) {
                    e.printStackTrace();
                }
            }
            /// @}
            riljLog("lookupOperatorNameFromNetwork in responseOperatorInfosWithAct()"
                    + ",updated longAlpha= " + strings[i + 0]
                    + ",shortAlpha= " + strings[i + 1]
                    + ",numeric=" + strings[i + 2]);

            /*
             * Operator name from SIM (EONS/CPHS) has highest priority to
             * display. This will be handled in GsmSST updateSpnDisplay()
             */
            /*
             * ALPS00353868: To get operator name from OPL/PNN/CPHS, which need
             * lac info
             */
            if ((lacValid == true) && (strings[i + 0] != null)) {
                int phoneId = mInstanceId;
                UiccController uiccController = UiccController.getInstance();
                SIMRecords simRecord = (SIMRecords) uiccController
                        .getIccRecords(mInstanceId, UiccController.APP_FAM_3GPP);
                int lacValue = -1;
                String sEons = null;
                String lac = lacStr.substring(lacIndex, lacIndex + 4);
                Rlog.d(RILJ_LOG_TAG, "lacIndex=" + lacIndex + " lacValue=" + lacValue
                        + " lac=" + lac + " plmn numeric=" + strings[i + 2]
                        + " plmn name" + strings[i + 0]);
                ...
                if (lac != "") {
                    ...
                    if (lacValue != 0xfffe) {
                        ...
                        if (sEons != null) {
                            ...
                            //[ALPS01858353]-Start: The CPHS operator name shall
    ...
    responseFemtoCellInfos(Parcel p) {
        ...
        for (int i = 0 ; i < strings.length ; i += 6) {
            ...
            if (strings[i + 1] != null
                ...
                /* // TODO: disabled for build pass. module to fix.
                strings[i + 1] = SpnOverride.getInstance().lookupOperatorName(
                        SubscriptionManager.getSubIdUsingPhoneId(mInstanceId), strings[i + 0],
                        true, mContext);
                */ // TODO: disabled for build pass. module to fix.
            }
            ...
            Rlog.d(RILJ_LOG_TAG, "property_name=" + property_name + ",basebandCapability="
                    + basebandCapability);
            ...
            Rlog.d(RILJ_LOG_TAG,
                    "FemtoCellInfo(" + strings[i + 3] + "," + strings[i + 4] + "," + strings[i + 5]
                            + "," + strings[i + 0] + "," + strings[i + 1] + "," + rat + ")"
                            + "hnbName=" + hnbName);
...
*/
        switch(request) {
            ...
            // M: CC33 LTE End.
            // M: For OP12 Start.
            case RIL_REQUEST_SYNC_APN_TABLE: return "RIL_REQUEST_SYNC_APN_TABLE";
            // M: For OP12 End.
            ...
            // M: VzW
            case RIL_REQUEST_SWITCH_MODE_FOR_ECC: return "RIL_REQUEST_SWITCH_MODE_FOR_ECC";
...
*/
        switch(request) {
            ...
            /// @}
            /// M: CC: Vzw ECC/hVoLTE redial
            case RIL_UNSOL_CALL_REDIAL_STATE: return "UNSOL_CALL_REDIAL_STATE";
            ...
            /// @}
            /* M: eMBMS feature */
            case RIL_UNSOL_EMBMS_SESSION_STATUS: return "RIL_UNSOL_EMBMS_SESSION_STATUS";
    ...
    public void setCdmaBroadcastConfig(CdmaSmsBroadcastConfigInfo[] configs, Message response) {
        ...
        for (CdmaSmsBroadcastConfigInfo config : configs) {
            for (int i = config.getFromServiceCategory(); i <= config
                    .getToServiceCategory(); i++) {
                processedConfigs.add(new CdmaSmsBroadcastConfigInfo(i, i, config.getLanguage(),
                        config.isSelected()));
            }
        }
    ...
    */
    ...
    vtDial(String address, int clirMode, UUSInfo uusInfo, Message result) {
        if (!PhoneNumberUtils.isUriNumber(address)) {
           ...
           RILRequest rr = RILRequest.obtain(RIL_REQUEST_VT_DIAL_WITH_SIP_URI, result);
    ...
    public void syncApnTable(String index, String apnClass, String apn, String apnType,
        ...
        RILRequest rr
                = RILRequest.obtain(RIL_REQUEST_SYNC_APN_TABLE, result);
        ...
        if (RILJ_LOGD) {
            riljLog(rr.serialString() + "> "
                + requestToString(rr.mRequest) + " " + index + " " + apnClass + " "
                + apn + " " + apnType + " " + apnEnable + " " + apnTime + " "
                + maxConn + " " + maxConnTime + " " + waitTime + " " + throttlingTime
                + " " + inactiveTimer);
        }
    ...
    public void syncDataSettingsToMd(boolean dataSetting, boolean dataRoamingSetting,
            Message result) {
        RILRequest rr
                = RILRequest.obtain(RIL_REQUEST_SYNC_DATA_SETTINGS_TO_MD, result);
        ...
        if (RILJ_LOGD) {
            riljLog(rr.serialString() + "> "
                + requestToString(rr.mRequest) + " " + dataSetting + " " + dataRoamingSetting);
        }
        send(rr);
    }
    // M: For OP12 End
    ...
    doGeneralSimAuthentication(int sessionId, int mode , int tag, String param1,
        ...
        if (RILJ_LOGD)
            riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) + ": " + "session = "
                    + sessionId + ",mode = " + mode + ",tag = " + tag + ", " + param1 + ", "
                    + param2);
...
//
//        if (RILJ_LOGD)
//            riljLog(rr.serialString() + "> iccOpenChannelWithSw: " + requestToString(rr.mRequest)
//                    + " " + AID);
//
//        send(rr);
//    }
    ...
    // MTK-START, SIM power
    /*
     * Allow to set sim power with indicated mode.
     * @param mode indicated sim power operation.
     *        RILConstants.SIM_POWER_OFF(0): power off (TODO, not support currenttly)
     *        RILConstants.SIM_POWER_ON(1): power on  (TODO, not support currenttly)
     *        RILConstants.SIM_POWER_RESET(2): reset sim
     * @param response the responding message.
     */
    @Override
    public void
    setSimPower(int mode, Message result) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SET_SIM_POWER, result);
...
//
//        if (RILJ_LOGD)
//            riljLog(rr.serialString() + "> " + requestToString(rr.mRequest) + ": " + nAction);
//
//        send(rr);
//    }
//
...
//    // Added by M end
    ...
    public void queryUPBAvailable(int eftype, int fileIndex, Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_QUERY_UPB_AVAILABLE, response);
        rr.mParcel.writeInt(2);
    ...
    public void readUPBEmailEntry(int adnIndex, int fileIndex, Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_READ_EMAIL_ENTRY, response);
        rr.mParcel.writeInt(2);
        rr.mParcel.writeInt(adnIndex);
        rr.mParcel.writeInt(fileIndex);
        if (RILJ_LOGD) riljLog(rr.serialString() + "> " + requestToString(rr.mRequest));
    ...
    public void readUPBSneEntry(int adnIndex, int fileIndex, Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_READ_SNE_ENTRY, response);
        rr.mParcel.writeInt(2);
        rr.mParcel.writeInt(adnIndex);
        rr.mParcel.writeInt(fileIndex);
        if (RILJ_LOGD) riljLog(rr.serialString() + "> " + requestToString(rr.mRequest));
    ...
    public void readUPBAnrEntry(int adnIndex, int fileIndex, Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_READ_ANR_ENTRY, response);
        rr.mParcel.writeInt(2);
        rr.mParcel.writeInt(adnIndex);
        rr.mParcel.writeInt(fileIndex);
        if (RILJ_LOGD) riljLog(rr.serialString() + "> " + requestToString(rr.mRequest));
    ...
    public void readUPBAasList(int startIndex, int endIndex, Message response) {
            RILRequest rr = RILRequest.obtain(RIL_REQUEST_READ_UPB_AAS_LIST, response);
            rr.mParcel.writeInt(2);
            rr.mParcel.writeInt(startIndex);
            rr.mParcel.writeInt(endIndex);
            if (RILJ_LOGD) riljLog(rr.serialString() + "> " + requestToString(rr.mRequest));
...
//    }
        ...
        synchronized (mDtmfReqQueue) {
            ...
            if (queueSize > 0) {
                ...
                if (rr2.mRequest == RIL_REQUEST_DTMF_START) {
                    ...
                    if (RILJ_LOGD) riljLog("DTMF queue isn't 0, first request is START, "
    ...
    public String lookupOperatorNameFromNetwork(long subId, String numeric,
        ...
        nitzOperatorNumeric = TelephonyManager.getTelephonyProperty(phoneId,
                TelephonyProperties.PROPERTY_NITZ_OPER_CODE, "");
        if ((numeric != null) && (numeric.equals(nitzOperatorNumeric))) {
            if (desireLongName == true) {
                nitzOperatorName = TelephonyManager.getTelephonyProperty(phoneId,
                        TelephonyProperties.PROPERTY_NITZ_OPER_LNAME, "");
            } else {
                nitzOperatorName = TelephonyManager.getTelephonyProperty(phoneId,
                        TelephonyProperties.PROPERTY_NITZ_OPER_SNAME, "");
            }
        }
        ...
        riljLog("lookupOperatorNameFromNetwork numeric= " + numeric + ",subId= " + subId
                + ",nitzOperatorNumeric= " + nitzOperatorNumeric + ",nitzOperatorName= "
                + nitzOperatorName);
    ...
    setNetworkSelectionModeManualWithAct(String operatorNumeric, String act, Message response) {
        ...
        rr.mParcel.writeString("0"); //the 3rd parameter is for MTK RIL to identify it shall
                                     //be processed as semi auto network selection mode or not
                ...
                switch (format) {
                    ...
                    case 2:
                        if (strings[i + 2] != null) {
                            ...
                            strOperName = SpnOverride.getInstance().lookupOperatorName(
                                    SubscriptionManager.getSubIdUsingPhoneId(mInstanceId),
                                    strings[i + 2], true, mContext);
                        }
                        break;
                    default:
                        break;
    ...
    setNetworkSelectionModeSemiAutomatic(String operatorNumeric, String act, Message response) {
        ...
        rr.mParcel.writeString(operatorNumeric);
        rr.mParcel.writeString(act);
        rr.mParcel.writeString("1"); //the 3rd parameter is for MTK RIL to identify it shall
                                     //be processed as semi auto network selection mode
...
//    }
    ...
    public void triggerModeSwitchByEcc(int mode, Message response) {
        RILRequest rr = RILRequest.obtain(RIL_REQUEST_SWITCH_MODE_FOR_ECC, response);
        rr.mParcel.writeInt(1);
        rr.mParcel.writeInt(mode);
        if (RILJ_LOGD) {
