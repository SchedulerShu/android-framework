...
public class SubscriptionController extends ISub.Stub {
    ...
    static final boolean ENGDEBUG = TextUtils.equals(android.os.Build.TYPE, "eng");
        ...
        try {
           ...
           // MTK-END
            ...
            try {
                if (cursor != null && cursor.moveToFirst()) {
                    do {
                        ...
                        // MTK-START
                        //It needs to set subId whether sSlotIdxToSubId caches the subId or not.
            ...
            if (setDisplayName) {
                ...
                String simNumeric = mTelephonyManager.getSimOperatorNumeric(subIds[0]);
                String simMvnoName;
    ...
    public int clearSubInfoUsingPhoneId(int phoneId) {
        ...
        try {
            ...
            setReadyState(false);
            sSlotIdxToSubId.remove(phoneId);
            for (int i = mActiveList.size() - 1; i >= 0; i--) {
                SubscriptionInfo record = mActiveList.get(i);
                if (record.getSimSlotIndex() == phoneId) {
                    mActiveList.remove(i);
                    if (DBG) {
                        logdl("[clearSubInfoUsingPhoneId]- clear phoneId =" + phoneId +
                                " i = " + i);
                    }
                    break;
                }
    ...
    public void clearDefaultsForInactiveSubIds() {
        ...
        try {
            ...
            if (!("OP09".equals(SystemProperties.get("persist.operator.optr"))
                 || "OP02".equals(SystemProperties.get("persist.operator.optr"))
                 || "OP01".equals(SystemProperties.get("persist.operator.optr")))) {
