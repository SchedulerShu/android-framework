    ...
    private void processSendRequest(Object obj) {
        ...
        switch (request.intValue()) {
                ...
                if (number == null || number.isEmpty()) {
                    if (mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_GSM) {
                        if ((mPhone instanceof GsmCdmaPhone)
                            && ((GsmCdmaPhone) mPhone).isSupportSaveCFNumber()) {
                            if (action == CF_ACTION_ENABLE ||
                                action == CF_ACTION_REGISTRATION) {
                                String getNumber =
                                    ((GsmCdmaPhone) mPhone).getCFPreviousDialNumber(cfReason);
            ...
            case SS_REQUEST_GET_CALL_BARRING: {
                ...
                if (((GsmCdmaPhone) mPhone).isOpNotSupportOCB(facility)) {
                    if (mIsTempVolteUser) {
                        if (resp != null) {
        ...
        switch (request.intValue()) {
                    ...
                    if (mPhone.getPhoneType() == PhoneConstants.PHONE_TYPE_GSM) {
                        if ((mPhone instanceof GsmCdmaPhone)
                            ...
                            if ((action == CF_ACTION_ENABLE)
                                || (action == CF_ACTION_REGISTRATION)) {
                                boolean ret =
                                    ((GsmCdmaPhone) mPhone).applyCFSharePreference(cfReason,
                                        number);
                                if (!ret) {
                                    Rlog.d(LOG_TAG, "applySharePreference false.");
                                }
                            } else if (action == CF_ACTION_ERASURE) {
                                ((GsmCdmaPhone) mPhone).clearCFSharePreference(cfReason);
                            }
                        }
                    }
                    // - [ALPS02301009]

                    if (((GsmCdmaPhone) mPhone).queryCFUAgainAfterSet()
            ...
            case SS_REQUEST_GET_CALL_BARRING: {
                ...
                if (ar.exception != null && ar.exception instanceof ImsException) {
                    ...
                    if (imsException.getCode() == ImsReasonInfo.CODE_UT_XCAP_403_FORBIDDEN) {
                        ...
                        if (((GsmCdmaPhone) mPhone).isOpTransferXcap404()) {
            ...
            case SS_REQUEST_GET_CALL_WAITING: {
                ...
                if (mPhone instanceof GsmCdmaPhone) {
                    ...
                    if (gsmCdmaPhone.getTbcwMode() == GsmCdmaPhone.TBCW_UNKNOWN) {
                        queryVolteUser = true;
                    }
                }
                ...
                if (queryVolteUser) {
                    ...
                    if (ar.exception == null) {
                        gsmCdmaPhone.setTbcwMode(GsmCdmaPhone.TBCW_OPTBCW_VOLTE_USER);
                        gsmCdmaPhone.setTbcwToEnabledOnIfDisabled();
                        if (reqCode.intValue() == SS_REQUEST_GET_CALL_WAITING) {
                            gsmCdmaPhone.getTerminalBasedCallWaiting(resp);
                        } else {
                            gsmCdmaPhone.setTerminalBasedCallWaiting(enable, resp);
                        }
                    } else if (imsException != null
                            && imsException.getCode() == ImsReasonInfo.CODE_UT_XCAP_403_FORBIDDEN) {
                        gsmCdmaPhone.setTbcwMode(GsmCdmaPhone.TBCW_OPTBCW_NOT_VOLTE_USER);
                        gsmCdmaPhone.setSystemProperty(PROPERTY_TERMINAL_BASED_CALL_WAITING_MODE,
            ...
            case SS_REQUEST_GET_CLIR: {
                ...
                if (((GsmCdmaPhone) mPhone).isOpNotSupportCallIdentity()) {
                    arException = new CommandException(
                            CommandException.Error.SPECAIL_UT_COMMAND_NOT_SUPPORTED);
                    arResult = null;
                } else {
    ...
    private CommandException getCommandException(ImsException imsException) {
        switch(imsException.getCode()) {
            case ImsReasonInfo.CODE_UT_XCAP_409_CONFLICT:
                if (((GsmCdmaPhone) mPhone).isEnableXcapHttpResponse409()) {
