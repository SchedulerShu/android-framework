...
public class DataSubSelector {
    ...
    //Add by MTK,@{
    private static final boolean MTK_C2K_SUPPORT = SystemProperties.get(
            "ro.boot.opt_c2k_support").equals("1");
    //}@
    ...
    private final static String USER_SELECT_DEFAULT_DATA = "persist.radio.user.select.data";
    private final static String NEED_TO_WAIT_IMSI = "persist.radio.need.wait.imsi";
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(TelephonyIntents.ACTION_SIM_STATE_CHANGED)) {
                ...
                if (simStatus.equals(IccCardConstants.INTENT_VALUE_ICC_IMSI)) {
                    ...
                    } else if (isNeedWaitImsi(NEED_TO_WAIT_IMSI)) {
                        log("NeedWaitImsi so check again");
                        if (OPERATOR_OP01.equals(mOperatorSpec)) {
                            setNeedWaitImsi(NEED_TO_WAIT_IMSI, Boolean.toString(false));
                            subSelectorForOp01(mIntent);
                        }
                    }
                ...
                boolean userSelected = SystemProperties.getBoolean(USER_SELECT_DEFAULT_DATA, false);
                log("mIsUserConfirmDefaultData/nDefaultDataSubId:" + userSelected
                        + "/" + nDefaultDataSubId);
                if (userSelected
                ...
                log("Subinfo Record Update: detectedType = " + detectedType);
                ...
                if (MTK_C2K_SUPPORT) {
                    ...
                    else if(mIsWaitIccid == true && mOperatorSpec.equals(OPERATOR_OP01) &&
                            detectedType == SubscriptionManager.EXTRA_VALUE_NOCHANGE){
                        mIsWaitIccid = false;
                        subSelectorForOp01(mIntent);
                        return;
                    }
                    else if(mOperatorSpec.equals(OPERATOR_OP01) &&
                            detectedType == SubscriptionManager.EXTRA_VALUE_NOCHANGE){
                            subSelectorForC2k6m(mIntent);
                    }
                }
            ...
            if (TelephonyIntents.ACTION_WORLD_MODE_CHANGED.equals(action)) {
                ...
                if (wmState == WorldMode.MD_WM_CHANGED_END) {
                    ...
                    for (int i = 0; i < mPhoneNum; i++) {
                        ...
                        log("mworldModeReceiver subid: " + subid);
                        if (subid == mLastValidDefaultDataSubId) {
                            setCapability(i);
                            log("mworldModeReceiver set phone id: " + i);
                            return;
                        }
                    }
    ...
    public DataSubSelector(Context context, int phoneNum) {
        ...
        mPhoneNum = phoneNum;
        mOperatorSpec = SystemProperties.get("persist.operator.optr", OPERATOR_OM);
    ...
    private void subSelectorForC2k6m(Intent intent) {
        ...
        for (int i = 0; i < mPhoneNum; i++) {
            currIccId[i] = SystemProperties.get(PROPERTY_ICCID[i]);
            if (currIccId[i] == null || "".equals(currIccId[i])) {
                log("C2K6M: error: iccid not found, wait for next sub ready");
                return;
            }
            if (defaultIccid.equals(currIccId[i])) {
                phoneId = i;
...
//            if (setCapability(phoneId)) {
                //When doing switching, setCapability will return false, default data will not
                //change, but sim switch will redo later, it causes main sim and default data unsync
                setCapability(phoneId);
...
//                mIsNeedWaitImsi = true;
    ...
    private void subSelectorForOp09(Intent intent) {
        ...
        /** Fix issue of can't open data when switch card under airplane mode.
         * Maybe have subs but have no iccids, this situation is treated as no sims.
         * Ex, as hot plugout SIM1, we will enter this funtion to set default data.
         * at this time SIM2 plugout too, we wiil do nothing as SIM2 may has no invalid
         * subid value and phoneid value.
         */
    ...
    private void updateDefaultDataProperty() {
        ...
        SubscriptionInfo subInfo = SubscriptionController.getInstance()
                .getActiveSubscriptionInfo(defaultSub, mContext.getOpPackageName());
        if (subInfo != null) {
        ...
        if (detectedType == SubscriptionManager.EXTRA_VALUE_NEW_SIM) {
            ...
            for (int i = 0; i < mPhoneNum; i++) {
                if ((newSimSlot & (1 << i)) != 0) {
                    ...
                    if (!newSimIccid.equals("N/A") && !newSimIccid.equals(defaultIccid)) {
        ...
        if (usimIndexList.size() >= 2) {
            ...
            if (op02CardCount == 1) {
                ...
                log("C3: More than two OP02 cards or other operator cards inserted," +
                        "Display dialog");
                mPrevDefaultDataSubId = SubscriptionManager.getDefaultDataSubscriptionId();
