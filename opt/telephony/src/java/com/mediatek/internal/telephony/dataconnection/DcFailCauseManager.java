...
public class DcFailCauseManager {
    ...
    private static final String[][] specificPLMN = {{"33402", "334020"},  // OP112
                                                    {"50501"},            // OP19
                                                    {"311480"}};          // OP12
    ...
    private enum retryConfigForOp112 {
        ...
        private retryConfigForOp112(int value) {
            this.value = value;
        }
    ...
    private boolean createGsmDCTExt(Phone phone) {
        ...
        if (mIsBsp == false) {
            try{
                mGsmDCTExt =
                    MPlugin.createInstance(IGsmDCTExt.class.getName(), phone.getContext());
                if (DBG) {
    ...
    public boolean isPermanentFailByOp(DcFailCause cause, Operator op) {
        boolean isPermanent = true; // defualt value is true.

        switch (op) {
            case OP19:
                if (isSpecificNetworkAndSimOperator(op)) {
                    for (int tempCause : OP19_FAIL_CAUSE_TABLE) {
                        DcFailCause dcFailCause = DcFailCause.fromInt(tempCause);
                        if (cause.equals(dcFailCause)) {
                            isPermanent = false;
                        }
                    }
                }
