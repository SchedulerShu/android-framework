...
///M: setTxPower @{
static jboolean android_net_wifi_setTxPowerEnabledCommand(JNIEnv* env, jclass cls, jboolean enable)
{
    ...
    if (ioctl(skfd, IOCTL_SET_INTS, &wrq) < 0) {
        ALOGE("setTxPowerEnabledCommand failed");
    } else {
        ALOGD("setTxPowerEnabledCommand succeed");
        result = JNI_TRUE;
    }
    close(skfd);
    return result;
...
static jboolean android_net_wifi_setTxPowerCommand(JNIEnv* env, jclass cls, jint offset)
{
    jboolean result = JNI_FALSE;
    struct iwreq wrq;
    int skfd;
    ...
    /* do IOCTL */
    if (ioctl(skfd, IOCTL_SET_INTS, &wrq) < 0) {
        ALOGE("setTxPowerCommand failed");
    } else {
        ALOGD("setTxPowerCommand succeed");
        result = JNI_TRUE;
    }
    close(skfd);
    return result;
