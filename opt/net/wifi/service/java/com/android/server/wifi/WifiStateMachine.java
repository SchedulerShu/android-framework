...
public class WifiStateMachine extends StateMachine implements WifiNative.WifiRssiEventHandler {
    ...
    ///M: Add for enabling verbose logging
    ...
    */
    ...
    ///M: for operator plugin @{
    private static final int M_CMD_UPDATE_SETTINGS                      = BASE + 170;
    private static final int M_CMD_UPDATE_SCAN_INTERVAL                 = BASE + 171;
    private static final int M_CMD_UPDATE_COUNTRY_CODE                  = BASE + 172;

    private static final int M_CMD_GET_CONNECTING_NETWORK_ID            = BASE + 176;
    private static final int M_CMD_UPDATE_RSSI                          = BASE + 177;

    private static final int M_CMD_GET_DISCONNECT_FLAG                  = BASE + 188;
    private static final int M_CMD_NOTIFY_CONNECTION_FAILURE            = BASE + 190;
    ///@}
    ...
    /* M: Added command */
    private static final int M_CMD_DO_CTIA_TEST_ON                      = BASE + 210;
    private static final int M_CMD_DO_CTIA_TEST_OFF                     = BASE + 211;
    private static final int M_CMD_DO_CTIA_TEST_RATE                    = BASE + 212;

    ///M: For adb shell stopscan command
    private static final int M_CMD_UPDATE_BGSCAN                        = BASE + 213;
    private static final int M_CMD_SET_TX_POWER_ENABLED                 = BASE + 214;
    private static final int M_CMD_SET_TX_POWER                         = BASE + 215;
    private static final int M_CMD_GET_WIFI_STATUS                      = BASE + 216;
    private static final int M_CMD_SET_POWER_SAVING_MODE                = BASE + 217;

    /** M: Set Driver Wowlan Mode  @{ */
    private static final int M_CMD_SET_WOWLAN_NORMAL_MODE               = BASE + 218;
    private static final int M_CMD_SET_WOWLAN_MAGIC_MODE                = BASE + 219;
    /** @} */

    private static final int M_CMD_FLUSH_BSS                            = BASE + 220;
    private static final int M_CMD_GET_TEST_ENV                         = BASE + 221;
    ///M: ALPS02279279 [Google Issue] Cannot remove configured networks when factory reset
    private static final int M_CMD_FACTORY_RESET                        = BASE + 222;
    private static final int M_CMD_SET_TDLS_POWER_SAVE                  = BASE + 223;

    private static final int M_CMD_UPDATE_SCAN_STRATEGY                 = BASE + 224;

    ///M: ALPS01986276 should enable EAP SIM in state machine
    private static final int M_CMD_ENABLE_EAP_SIM_CONFIG_NETWORK        = BASE + 225;
    ...
    private static final int M_CMD_IP_REACHABILITY_MONITOR_TIMER        = BASE + 235;
    ...
    public AtomicBoolean enableIpReachabilityMonitor = new AtomicBoolean(true);
    public AtomicBoolean enableIpReachabilityMonitorEnhancement = new AtomicBoolean(true);
    ///@}
    ...
    /// M: for Don't Scan & Reconnect Strategy Feature. For features, like wfd,
    /// scan or reconnect could effect performance, therefore disable scan & reconnect during
    /// wfd connection. WPS connect and user trigger connect won't effect by scan strategy
    ...
    ///M: Add for query message whether existed in queue or not
    ...
    public WifiStateMachine(Context context, FrameworkFacade facade, Looper looper,
        ...
        ///M: for operator plugin @{
        mWifiQualifiedNetworkSelector.setWifiFwkExt(mWifiFwkExt);
        ///@}
        ...
        ///M: IpReachabilityMonitor Enhancement @{
        enableIpReachabilityMonitor.set(mContext.getResources().getBoolean(
                    com.mediatek.internal.R.bool.
                    config_enable_ip_reachability_monitor));
        log("enableIpReachabilityMonitor: " +
                enableIpReachabilityMonitor.get());

        enableIpReachabilityMonitorEnhancement.set(mContext.getResources().getBoolean(
                    com.mediatek.internal.R.bool.
                    config_enable_ip_reachability_monitor_enhancement));
        log("enableIpReachabilityMonitorEnhancement: " +
                enableIpReachabilityMonitorEnhancement.get());
        ///@}
        ...
        ///M: Add for WAPI and EAT-FAST @{
        mWifiMonitor.registerHandler(mInterfaceName, WifiMonitor.WAPI_NO_CERTIFICATION_EVENT,
                getHandler());
        mWifiMonitor.registerHandler(mInterfaceName, WifiMonitor.NEW_PAC_UPDATED_EVENT,
                getHandler());
        ///@}
        ///M: Add for whole chip reset event @{
        mWifiMonitor.registerHandler(mInterfaceName, WifiMonitor.WHOLE_CHIP_RESET_FAIL_EVENT,
                getHandler());
        ///@}
        ///M: Add for TDLS @{
        mWifiMonitor.registerHandler(mInterfaceName, WifiMonitor.TDLS_CONNECTED_EVENT,
                getHandler());
        mWifiMonitor.registerHandler(mInterfaceName, WifiMonitor.TDLS_DISCONNECTED_EVENT,
                getHandler());
        ///@}
    ...
    public boolean isSupplicantTransientState() {
        ...
        ///M: ALPS02823640 Prevent auto roam when supplicant is ASSOCIATED state
     ...
     */
    ///M: ALPS02800523 extends to multiple sim
    ...
    public void dump(FileDescriptor fd, PrintWriter pw, String[] args) {
        try {
            ...
            ///M: ALPS02941902 for prevent concurrency exception
        ...
        if (DBG) {
            switch (message.what) {
                ...
                case CMD_CONFIG_ND_OFFLOAD:
                    if (!MDBG) {
                        // Not important message, don't need to print log
                        break;
                    }
                default:
    ...
    private void handleNetworkDisconnect() {
        ...
        if (hasCustomizedAutoConnect()) {
            ...
            Log.d(TAG, "handleNetworkDisconnect, state:" +
                state + ", mDisconnectOperation:" + mDisconnectOperation);
    ...
    class DefaultState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case M_CMD_SET_TDLS_POWER_SAVE:
                ...
                case WifiManager.SET_WIFI_NOT_RECONNECT_AND_SCAN:
                    ...
                    if (message.arg1 == 1 || message.arg1 == 2) {
                        ...
                        sendMessage(M_CMD_UPDATE_SCAN_STRATEGY);
                        if (mWifiConnectivityManager != null) {
                            mWifiConnectivityManager.handleScanStrategyChanged();
                        }
                    } else {
                    ...
                    if (null != networks) {
                        ...
                        for (WifiConfiguration network : networks) {
                            if (mWifiConfigManager.isSimConfig(network)
                                ...
                                mWifiConfigManager.updateNetworkSelectionStatus(eapSimConfig,
                                        WifiConfiguration.NetworkSelectionStatus
                                        .NETWORK_SELECTION_ENABLE);
                                isSimConfigExisted = true;
                            }
                        }
                        ///M: start scan immediately
                        if (isSimConfigExisted) {
                            if (mWifiConnectivityManager != null) {
                                mWifiConnectivityManager.handleScanStrategyChanged();
                            }
                        }
                ...
                case CMD_START_SUPPLICANT:
                    if (mWifiNative.loadDriver()) {
                        try {
                            ...
                            try {
                                if (mPropertyService.get("ro.have_aee_feature", "").equals("1")) {
            ...
            switch(message.what) {
                ...
                case CMD_RESET_SIM_NETWORKS:
                    log("resetting EAP-SIM/AKA/AKA' networks since SIM was removed"
                            + ", simSlot: " + message.arg1);
                    ///M: ALPS02800523 extends to multiple sim
                    mWifiConfigManager.resetSimNetworks((int)message.arg1);
                    break;
                case M_CMD_DO_CTIA_TEST_ON:
        ...
        public boolean processMessage(Message message) {
            ...
            switch(message.what) {
                ...
                case CMD_ENABLE_AUTOJOIN_WHEN_ASSOCIATED:
                    ...
                    break;
                ///M: Add for setting power saving and TDLS power saving @{
                case M_CMD_SET_POWER_SAVING_MODE:
                    mWifiNative.setPowerSave(message.arg1 == 1);
                    break;
                case M_CMD_SET_TDLS_POWER_SAVE:
                    mWifiNative.setTdlsPowerSave(message.arg1 == 1);
                    break;
                ///@}
                default:
                    return NOT_HANDLED;
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case CMD_ADD_OR_UPDATE_NETWORK:
                    ...
                    ///M: Check scan result is GBK or not, record in config.isGbkEncoding @{
                    checkGbkEncoding(config);
                    ///@}
                ...
                case CMD_REMOVE_NETWORK:
                    ...
                    ///M: for operator plugin @{
                    if (hasCustomizedAutoConnect()) {
                        mWifiConfigManager.removeDisconnectNetwork(message.arg1);
                ...
                case CMD_ENABLE_NETWORK:
                    ...
                    if (hasCustomizedAutoConnect() && message.arg2 == 0) {
                        if (!mWifiFwkExt.shouldAutoConnect()) {
                            ...
                            List<Integer> disconnectNetworks =
                                mWifiConfigManager.getDisconnectNetworks();
                            if (disconnectNetworks.contains(message.arg1)) {
                                Log.d(TAG,
                                    "Network " + message.arg1 +
                                    " is disconnected actively," +
                                    "ignore the enable network operation!");
                                replyToMessage(message, message.what, SUCCESS);
                                break;
                            }
                        }
                ...
                case WifiManager.DISABLE_NETWORK:
                    if (mWifiConfigManager.updateNetworkSelectionStatus(message.arg1,
                        ...
                        ///M: for operator plugin @{
                        if (hasCustomizedAutoConnect()) {
                            mWifiConfigManager.addDisconnectNetwork(message.arg1);
                ...
                case WifiMonitor.SUP_REQUEST_IDENTITY:
                    ...
                    if (targetWificonfiguration != null
                        ...
                        if (tm != null) {
                            ...
                            if (tm.getDefault().getPhoneCount() >= 2) {
                                int slotId = WifiConfigurationUtil.
                                    getIntSimSlot(targetWificonfiguration);
                                int subId = getSubId(slotId);
                                log("simSlot: " + targetWificonfiguration.simSlot + " " + slotId
                                        + "subId: " + subId);
                                if (subId == GET_SUBID_NULL_ERROR) {
                                    // If sim slot is unspecified, should use default behavior
                                    imsi = tm.getSubscriberId();
                                    if (tm.getSimState() == TelephonyManager.SIM_STATE_READY) {
                                        mccMnc = tm.getSimOperator();
                                    }
                                    // Update config's simSlot
                                    log("config.simSlot is unspecified(-1)"
                                            + ", so it should be changed to default sim slot");
                                    targetWificonfiguration.simSlot
                                        = "\"" + tm.getDefaultSim() + "\"";
                                    if (setSimSlotNative(targetWificonfiguration)) {
                                        log("config.simSlot is changed to "
                                                + targetWificonfiguration.simSlot);
                                    }
                                } else {
                    ...
                    if (!identitySent) {
                        ...
                        ///M: for operator plugin @{
                            ...
                            log("Skip SUP_REQUEST_IDENTITY disconnect for customization!");
                        }
                        ///@}
                    }
                    break;
                ...
                case CMD_AUTO_CONNECT:
                    ...
                    if (mWifiConfigManager.isSimConfig(config)) {
                        if(!mIccState.equals(IccCardConstants.INTENT_VALUE_ICC_LOADED)){
                            ...
                            ///M: sim card is ready, check sim card changed or not
                            ///   if sim card changed, clear identity and annonymous_id
                            if (checkOrCleanIdentity(config)) {
                                logd("checkOrCleanIdentity success, continue auto connect");
                            } else {
                                logd("checkOrCleanIdentity fail, exist auto connect");
                                break;
                            }
                        }
                    }
                    ...
                    if (hasCustomizedAutoConnect()) {
                        tmpResult = mWifiConfigManager.enableNetwork(config,
                            true,
                            lastConnectUid);
                    } else {
                        tmpResult = mWifiConfigManager.selectNetwork(config,
                ...
                case WifiManager.CONNECT_NETWORK:
                    ...
                    if (mWifiConfigManager.selectNetwork(config, /* updatePriorities = */ true,
                        ...
                        if (hasCustomizedAutoConnect()) {
                            mDisconnectOperation = true;
                            mScanForWeakSignal = false;
                            mWifiConfigManager.removeDisconnectNetwork(netId);
                        }
                        ///@}
                        ///@}
                        if (didDisconnect) {
                            ...
                            transitionTo(mDisconnectingState);
                                ...
                                ///M: ALPS02850241 Google deault timming issue
                                (getCurrentState() == mConnectedState
                ...
                case WifiManager.FORGET_NETWORK:
                    ...
                    if (mWifiConfigManager.forgetNetwork(message.arg1)) {
                        ...
                        if (message.arg1 == mLastExplicitNetworkId) {
                            mLastExplicitNetworkId = INVALID_NETWORK_ID;
                            mConnectNetwork = false;
                            mSupplicantStateTracker.sendMessage(
                                WifiManager.FORGET_NETWORK, message.arg1);
                        }
                        ///@}
                ...
                case WifiMonitor.WAPI_NO_CERTIFICATION_EVENT:
                    ...
                    mContext.sendBroadcastAsUser(new Intent(WifiManager.NO_CERTIFICATION_ACTION),
                            UserHandle.ALL);
                    break;
                case WifiMonitor.NEW_PAC_UPDATED_EVENT:
                    Log.d(TAG, "EAP-FAST new pac updated!");
                    mContext.sendBroadcastAsUser(new Intent(WifiManager.NEW_PAC_UPDATED_ACTION),
                            UserHandle.ALL);
                    break;
                case WifiManager.STOP_PPPOE:
                    if (mMtkCtpppoe) {
                ...
                case M_CMD_FACTORY_RESET:
                    ...
                    if (networks != null) {
                        for (WifiConfiguration c : networks) {
                            if (!mWifiConfigManager.canModifyNetwork(uid, c.networkId,
                                        /* onlyAnnotate */ false)) {
                                logw("Not authorized to remove network "
                                        + " cnid=" + c.networkId
                                        + " uid=" + uid);
                            ...
                            ok = mWifiConfigManager.removeNetwork(c.networkId);
                            ///M: for operator plugin @{
                            if (hasCustomizedAutoConnect()) {
                                mWifiConfigManager.removeDisconnectNetwork(c.networkId);
                ...
                case WifiMonitor.TDLS_DISCONNECTED_EVENT:
                    ...
                    if (tokens.length >= 2) {
                        final String PEER_STR = "peer=";
                        final int peerStrLen = PEER_STR.length();
                        String peer;
                        String token = tokens[1];
                        if (token.startsWith(PEER_STR)) {
                            peer = new String(token.getBytes(), peerStrLen,
                                    token.length() - peerStrLen);
                            logd("peer: " + peer);
                            sendTdlsEventBroadcast(
                                    message.what == WifiMonitor.TDLS_CONNECTED_EVENT, peer);
                        } else {
                            loge("not start with \"peer\", format is wrong");
                        }
                    } else {
                        loge("token length < 2, format is wrong");
                    }
                    break;
                ///@}
                default:
            ...
            switch (message.what) {
                ...
                case CMD_IP_REACHABILITY_LOST:
                    ...
                    if (isTemporarilyDontReconnectWifi()) {
                        log("isTemporarilyDontReconnectWifi is true, " +
                                "ignore CMD_IP_REACHABILITY_LOST");
                    } else if (enableIpReachabilityMonitor()) {
                        ///M: ALPS02550356 Avoid frequently disconnect becasue of
                        ///   ip reachability lost @{
                        Log.d(TAG, "mIsListeningIpReachabilityLost: "
                                + mIsListeningIpReachabilityLost);
                        if (!enableIpReachabilityMonitorEnhancement()
                                || mIsListeningIpReachabilityLost) {
                            if (DBG && message.obj != null) log((String) message.obj);
                            handleIpReachabilityLost();
                            transitionTo(mDisconnectingState);
                        } else {
                            ///M: ALPS02550356 Avoid frequently disconnect
                            //    caused by ip reachability lost @{
                            Log.d(TAG, "mIsListeningIpReachabilityLost: "
                                    + mIsListeningIpReachabilityLost);
                        ...
                        Log.d(TAG, "Ignore CMD_IP_REACHABILITY_LOST " +
                                "due to enableIpReachabilityMonitor is off");
                    }
                    break;
                case CMD_DISCONNECT:
                    mWifiNative.disconnect();
                ...
                case WifiMonitor.NETWORK_CONNECTION_EVENT:
                    ...
                    ///M: IpReachabilityMonitor Enhancement @{
                    if (enableIpReachabilityMonitorEnhancement()) {
                        //start listening ip reachability lost
                        startListenToIpReachabilityLost();
                        Log.d(TAG, "driver roaming, " +
                                "start to listen ip reachability lost for 10 sec, " +
                ...
                case CMD_RSSI_POLL:
                    if (message.arg1 == mRssiPollToken) {
                        if (mWifiConfigManager.mEnableChipWakeUpWhenAssociated.get()) {
                            ...
                            fetchRssiLinkSpeedAndFrequencyNative();
                            ///M: for operator plugin, add argument hasCustomizedAutoConnect()
                            mWifiScoreReport =
                ...
                case CMD_RESET_SIM_NETWORKS:
                    if (mLastNetworkId != WifiConfiguration.INVALID_NETWORK_ID) {
                        ...
                        if (mWifiConfigManager.isSimConfig(config)
                                && configSimSlot == removedSimSlot) {
                            log("config.simSlot: " + config.simSlot + "," + configSimSlot
                                    + " equals removedSimSlot: " + removedSimSlot);
                        ///@}
                            mWifiNative.disconnect();
                            transitionTo(mDisconnectingState);
                        }
                ...
                case WifiManager.START_PPPOE:
                    ...
                    Log.d(TAG, "mPppoeInfo.status:" + mPppoeInfo.status +
                            ", config:" + (PPPOEConfig) message.obj);
                ...
                case M_CMD_IP_REACHABILITY_MONITOR_TIMER:
                    if (message.arg1 == ipReachabilityMonitorCount) {
                        Log.d(TAG, "IpReachabilityMonitor timer time out, count: "
                                + ipReachabilityMonitorCount);
                        mIsListeningIpReachabilityLost = false;
                    } else {
                        Log.d(TAG, "IpReachabilityMonitor count mismatch, count: "
        ...
        public void enter() {
            ...
            if (!mWifiConfigManager.isUsingStaticIp(mLastNetworkId)) {
                ...
                if (config.ipAddress == null) {
                    ...
                    ///@}
                    ///M: ALPS02810261 Delay static ip start provisioning due to timing issue @{
                    try {
                        Thread.sleep(500);
                    } catch (Exception e) {
                    }
            ...
            switch (message.what) {
                ...
                case WifiMonitor.SUPPLICANT_STATE_CHANGE_EVENT:
                    ...
                    if (stateChangeResult.state == SupplicantState.ASSOCIATED) {
                        ...
                        ///M: ALPS02825547 mWifiInfo.SSID is not sync with associated AP,
                        //    due to link configuration roaming @{
                        if (stateChangeResult.wifiSsid != null
                                    && !stateChangeResult.wifiSsid.toString().isEmpty()) {
                            mWifiInfo.setSSID(stateChangeResult.wifiSsid);
                        }
                        ///@}
                    }
    ...
    class ConnectedState extends State {
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case CMD_ASSOCIATED_BSSID:
                    if (mLastBssid != null && mLastBssid.equals(message.obj)) {
                        log("bssid is the same, it isn't roaming");
                    } else{
                ...
                case WifiMonitor.NETWORK_DISCONNECTION_EVENT:
                    ...
                    if (mScreenOn
                            ...
                            && ((message.arg2 != 3 /* reason cannot be 3, i.e. locally generated */
                                    ///M: ALPS02711675 If STA encounter beacon timeout
                                    ///   (reason is 100), wifi should not goto debouncing.
                                    && message.arg2 != 100)
                ...
                case CMD_AUTO_ROAM:
                    ...
                    */
                    ...
                    if (mLastNetworkId != netId) {
                        ...
                        boolean tmpResult = false;
                        if (hasCustomizedAutoConnect()) {
                            tmpResult = mWifiConfigManager.enableNetwork(config, true,
                ...
                case EVENT_UPDATE_DNS:
                    ...
                    for (int i = 0; i < pppoeDnses.size(); i++) {
                        ...
                        mPropertyService.set("net.dns" + (i + 1), pppoeDnses.get(i));
                    }
                    break;
                ///@}
                default:
        ...
        public boolean processMessage(Message message) {
            ...
            switch (message.what) {
                ...
                case WifiMonitor.SUPPLICANT_STATE_CHANGE_EVENT:
                    ...
                    transitionTo(mDisconnectedState);
                    break;
                case M_CMD_UPDATE_SCAN_STRATEGY:
                    deferMessage(message);
                    break;
                default:
                    return NOT_HANDLED;
    ...
    String getGsmSimAuthResponse(String[] requestData, TelephonyManager tm) {
        return getGsmSimAuthResponse(requestData, tm, INVALID_NETWORK_ID);
    }
    ...
    String getGsmSimAuthResponse(String[] requestData, TelephonyManager tm, int netId) {
        ...
        for (String challenge : requestData) {
            ...
            ///M: Extend to multiple sim card
            String tmResponse = getIccAuthentication(tm.APPTYPE_USIM,
                    tm.AUTHTYPE_EAP_SIM, base64Challenge, tm, netId);
            if (tmResponse == null) {
                /* Then, in case of failure, issue may be due to sim type, retry as a simple sim
                 */
                appType = tm.APPTYPE_SIM;
                ///M: Extend to multiple sim card
                tmResponse = getIccAuthentication(tm.APPTYPE_SIM,
                        tm.AUTHTYPE_EAP_SIM, base64Challenge, tm, netId);
            }
            ...
            if (appType == tm.APPTYPE_USIM) {
                ...
                kc = makeHex(result, 1 + kc_offset, kc_len);
            } else if (appType == tm.APPTYPE_SIM) {
                if (result.length < 12) {
                    loge("malfomed response - " + tmResponse);
                    return null;
                }
    ...
    void handle3GAuthRequest(SimAuthRequestData requestData) {
        ...
        if (rand != null && authn != null) {
            ...
            if (tm != null) {
                ///M: Extend to multiple sim card
                tmResponse = getIccAuthentication(tm.APPTYPE_USIM,
                        tm.AUTHTYPE_EAP_AKA, base64Challenge,
                        tm, requestData.networkId);
                logv("Raw Response - " + tmResponse);
            } else {
                loge("could not get telephony manager");
            }
    ...
    public boolean syncSetTxPowerEnabled(AsyncChannel channel, boolean enable) {
        Message resultMsg = channel.sendMessageSynchronously(M_CMD_SET_TX_POWER_ENABLED,
                enable ? 1 : 0);
        boolean result = (resultMsg.arg1 != FAILURE);
        resultMsg.recycle();
        return result;
    ...
    public List<Integer> syncGetDisconnectNetworks() {
        return mWifiConfigManager.getDisconnectNetworks();
    }

    public boolean isNetworksDisabledDuringConnect() {
        return
            (mSupplicantStateTracker.isNetworksDisabledDuringConnect() && isExplicitNetworkExist())
    ...
    private class PppoeHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            ...
            int result = NetworkUtils.runPPPOE(mInterfaceName, mPppoeConfig.timeout,
                    mPppoeConfig.username, mPppoeConfig.password,
                    mPppoeConfig.lcp_echo_interval, mPppoeConfig.lcp_echo_failure,
                    mPppoeConfig.mtu, mPppoeConfig.mru,
    ...
    private void stopPPPoE() {
        ...
        try {
            mNwService.removeInterfaceFromNetwork(mPppoeLinkProperties.getInterfaceName(),
                    PPPOE_NETID);
    ...
    private void handleSuccessfulPppoeConfiguration(DhcpResults pppoeResult) {
        mPppoeLinkProperties = pppoeResult.toLinkProperties("wlan0");
        Log.d(TAG, "handleSuccessfulPppoeConfiguration, mPppoeLinkProperties:"
                + mPppoeLinkProperties);
        ...
        for (int i = 0; i < dnsArr.length; i++) {
            ...
            mPropertyService.set("net.dns" + (i + 1), dnsArr[i]);
        }
        try {
            mNwService.createPhysicalNetwork(PPPOE_NETID, null);
            mNwService.addInterfaceToNetwork("wlan0", PPPOE_NETID);
    ...
    public boolean isTemporarilyDontReconnectWifi() {
        log("stopReconnectWifi" + " StopScan=" + mStopScanStarted.get() +
            " mDontReconnectAndScan=" + mDontReconnectAndScan.get());
        if (mStopScanStarted.get() || mDontReconnectAndScan.get()) {
            return true;
        }
        return false;
    }
    ...
    ///M: EAP SIM/AKA/AKA' extend to multiple sim card @{
    ...
    public String getIccAuthentication(
            int appType, int authType,
            String base64Challenge, TelephonyManager tm, int netId) {
        ...
        if (netId != INVALID_NETWORK_ID) {
            ...
            if (tm.getDefault().getPhoneCount() >= 2 && config != null) {
                int subId = getSubId(WifiConfigurationUtil.getIntSimSlot(config));
                if (subId != GET_SUBID_NULL_ERROR) {
                    //log("subId: " + subId + ", appType: "+ appType
                    //        + ", authType: " + authType + ", challenge: " + base64Challenge);
                    tmResponse = tm.getIccAuthentication(subId, appType,
                            authType, base64Challenge);
                    return tmResponse;
                }
            }
        }
        tmResponse = tm.getIccAuthentication(appType, authType, base64Challenge);
        return tmResponse;
    }
    ...
    private void sendTdlsEventBroadcast(boolean isConnectedEvent, String macAddress) {
        ...
        Intent intent;
        if (isConnectedEvent) {
            intent = new Intent(WifiManager.TDLS_CONNECTED_ACTION);
        } else {
            intent = new Intent(WifiManager.TDLS_DISCONNECTED_ACTION);
        }
        if (macAddress != null) {
            intent.putExtra(WifiManager.EXTRA_TDLS_BSSID, macAddress);
        }
        mContext.sendBroadcastAsUser(intent, UserHandle.ALL);
    }
    ///@}
    ...
    ///M: IpReachabilityMonitor Enhancement @{
    ///M: No need to trigger probeAll because Google default will trigger probeAll
    ///   when supplicant state is COMPLETED.
    ///   Start listening to CMD_IP_REACHABILITY_LOST for timerMs and
    ///   increment the counter.
    private void startListenToIpReachabilityLost() {
        mIsListeningIpReachabilityLost = true;
        ipReachabilityMonitorCount++;
        sendMessageDelayed(obtainMessage(M_CMD_IP_REACHABILITY_MONITOR_TIMER,
                    ipReachabilityMonitorCount, 0), IP_REACHABILITY_MONITOR_TIMER_MSEC);
    }

    private boolean enableIpReachabilityMonitor() {
        boolean enable = mPropertyService.get("persist.wifi.IRM.enable", "1").equals("1");
        log("enable IpReachabilityMonitor Enhancement config: "
                + enableIpReachabilityMonitor.get()
                + ", SystemProperty: " + enable);
        if (enableIpReachabilityMonitor.get() || enable) {
            return true;
        } else {
    ...
    private boolean enableIpReachabilityMonitorEnhancement() {
        boolean enable = mPropertyService.get("persist.wifi.IRM.enhancement", "1").equals("1");
        log("enable IpReachabilityMonitor Enhancement config: "
                + enableIpReachabilityMonitorEnhancement.get()
                + ", SystemProperty: " + enable);
        if (enableIpReachabilityMonitorEnhancement.get() || enable) {
            return true;
        } else {
    ...
    ///M: ALPS02548518 EAP SIM/AKA/AKA' sim swap @{
    private boolean checkOrCleanIdentity(WifiConfiguration config) {
        int eapMethod = WifiEnterpriseConfig.Eap.NONE;
        if (config != null
                && config.enterpriseConfig != null) {
            eapMethod = config.enterpriseConfig.getEapMethod();
        } else {
        ...
        if (tm != null) {
            ...
            if (!identity.isEmpty()) {
                // check identity is same or not
                if (config.enterpriseConfig.getIdentity().equals(identity)) {
                    log("same card");
                } else {
                    log("different identity: " + config.enterpriseConfig.getIdentity()
                            + "new identity: " + identity);
                    // clear old identity
                    mWifiConfigManager.resetSimNetwork(config);
    ...
    ///M: for operator plugin @{
    /**
     * In mWifiFwkExt.init(), it has mWifiManager = (WifiManager) mContext.getSystemService
     * (Context.WIFI_SERVICE); We have to make sure that WifiService is added into SystemService
    ...
    public void autoConnectInit() {
        ...
        mWifiConfigManager.setWifiFwkExt(mWifiFwkExt);
    ...
    private int getHighPriorityNetworkId() {
        ...
        int rssi = MIN_RSSI;
        String ssid = null;
        List<WifiConfiguration> networks = mWifiConfigManager.getConfiguredNetworks();
        ...
        if (null != networks) {
            if (mWifiFwkExt.shouldAutoConnect()) {
                if (!isConnecting) {
                    ...
                    List<Integer> disconnectNetworks = mWifiConfigManager.getDisconnectNetworks();
                    for (WifiConfiguration network : networks) {
                        if (network.networkId != mLastNetworkId
                            && !disconnectNetworks.contains(network.networkId)) {
                            mWifiConfigManager.enableNetwork(network, false,
                                    WifiConfiguration.UNKNOWN_UID);
                        }
                    }
                }
