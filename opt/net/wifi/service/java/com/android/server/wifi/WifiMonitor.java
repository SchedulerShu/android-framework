...
public class WifiMonitor {
     ...
     */
    ///M: ALPS02711675 If disconnect event didn't have "locally_generated" will cause match fail
    ...
    /**
     * Regex pattern for p2pErrorEx from p2p supplicant error string.
     * Matches a strings like the following:<pre>
     * XXXX-XXXX-XXXX status=X</pre>
     */
    private static final Pattern p2pErrorExPattern1 =
            Pattern.compile("status=([0-9]{1,2})");

    /**
     * Regex pattern for p2pErrorEx from p2p supplicant error string.
     * Matches a strings like the following:<pre>
     * XXXX-XXXX-XXXX status=X peer_oper_freq=XXXXX</pre>
     */
    private static final Pattern p2pErrorExPattern2 =
            Pattern.compile("status=([0-9]{1,2}) " + "peer_oper_freq=(-?[0-9]{1,4})");

    /**
     * Regex pattern for p2pErrorEx from p2p supplicant error string.
     * Matches a strings like the following:<pre>
     * XXXX-XXXX-XXXX reason=FREQ_CONFLICT peer_oper_freq=XXXX/pre>
     */
    private static final Pattern p2pErrorExPattern3 =
            Pattern.compile("reason=FREQ_CONFLICT peer_oper_freq=(-?[0-9]{1,4})");
    /** @} */
    ///@}
   ...
   /* Network connection completed */
    ...
    ///M: Add for WAPI
    public static final int WAPI_NO_CERTIFICATION_EVENT          = BASE + 80;
    public static final int NEW_PAC_UPDATED_EVENT                = BASE + 81;
    ///M: whole chip reset fail
    public static final int WHOLE_CHIP_RESET_FAIL_EVENT          = BASE + 82;

    /** M: TDLS event  @{ */
    public static final int TDLS_CONNECTED_EVENT                   = BASE + 83;
    public static final int TDLS_DISCONNECTED_EVENT                = BASE + 84;
    /** @} */
    ...
    ///M: enhance frequency conflict
    private static int sP2pOperFreq = -1;
    ...
    private boolean dispatchEvent(String eventStr, String iface) {
        ...
        if (!eventStr.startsWith(EVENT_PREFIX_STR)) {
            ...
            ///M: Add for EAP-FAST
                ...
                sendMessage(iface, NEW_PAC_UPDATED_EVENT);
            ///M: whole chip reset fail
            } else if (eventStr.startsWith(WHOLE_CHIP_RESET_FAIL_STRING)) {
                sendMessage(iface, WHOLE_CHIP_RESET_FAIL_EVENT);
            }
            ///M: TDLS event @{
            else if (eventStr.startsWith(TDLS_CONNECTED_EVENT_STR)){
                Log.i(TAG, "TDLS_CONNECTED_EVENT");
                sendMessage(iface, TDLS_CONNECTED_EVENT, eventStr);
            } else if (eventStr.startsWith(TDLS_DISCONNECTED_EVENT_STR)){
                Log.i(TAG, "TDLS_DISCONNECTED_EVENT");
                sendMessage(iface, TDLS_DISCONNECTED_EVENT, eventStr);
            }
            ///@}
            else {
        ...
        */
        ...
        if ((event == SSID_TEMP_DISABLE)||(event == SSID_REENABLE)) {
            ...
            ///M: ALPS02829088 avoid long time to enable wifi @{
            // Notify and exit
            sendMessage(iface, SUP_DISCONNECTION_EVENT, eventLogCounter);
    ...
    private void handleEvent(int event, String remainder, String iface) {
        ...
        switch (event) {
            ...
            case NO_CERTIFICATION:
                sendMessage(iface, WAPI_NO_CERTIFICATION_EVENT);
                break;
            ///@}
...
///M: @{
    /* P2P-GROUP-REMOVED p2p0 client reason=FREQ_CONFLICT
       P2P-GROUP-REMOVED p2p0 client reason=FREQ_CONFLICT peer_oper_freq=2412 */
    private P2pStatus p2pError_GroupRemove(String dataString) {
        P2pStatus err = P2pStatus.UNKNOWN;
        String[] tokens = dataString.split(" ");

        if (tokens.length < 3) return err;
        ...
        if (err == P2pStatus.NO_COMMON_CHANNEL) {
            /* Parsing peer_oper_freq=2412*/
            if (tokens.length > 4) {
                ...
                if (!tokens[4].startsWith("peer_oper_freq")) {
                    ...
                    sP2pOperFreq = Integer.valueOf(freqNameValue[1]);
                }
    ...
    /** M: enhance frequency conflict @{ */
    /* P2P-INVITATION-RESULT status=7 peer_oper_freq=2412
       P2P-GO-NEG-FAILURE status=7 peer_oper_freq=5805
       P2P-GO-NEG-FAILURE status=7 peer_oper_freq=-1
       P2P-GROUP-FORMATION-FAILURE reason=FREQ_CONFLICT peer_oper_freq=2462*/
    private P2pStatus p2pErrorEx(String dataString) {
        P2pStatus status = P2pStatus.UNKNOWN;
        Matcher match;

        match = p2pErrorExPattern2.matcher(dataString);
        if (match.find()) {
            status = P2pStatus.valueOf(Integer.valueOf(match.group(1)));
            sP2pOperFreq = Integer.valueOf(match.group(2));
        } else {
        ...
        if (match.find()) {
            ...
            sP2pOperFreq = Integer.valueOf(match.group(1));
        }
    ...
    private void handleP2pEvents(String dataString, String iface) {
        if (dataString.startsWith(P2P_DEVICE_FOUND_STR)) {
            ...
            P2pStatus p2pStatus = p2pErrorEx(dataString);
            sendMessage(iface, P2P_GO_NEGOTIATION_FAILURE_EVENT, sP2pOperFreq, 0, p2pStatus);
            sP2pOperFreq = -1;
            /** @} */
            ...
            P2pStatus p2pStatus = p2pErrorEx(dataString);
            sendMessage(iface, P2P_GROUP_FORMATION_FAILURE_EVENT, sP2pOperFreq, 0, p2pStatus);
            sP2pOperFreq = -1;
            /** @} */
            ...
            /*M: ALPS00677009: parse error reason to wifi p2p framework*/
            /** M: enhance frequency conflict @{ */
            //WifiP2pGroup group = getWifiP2pGroup(dataString);
            //if (group != null) sendMessage(iface, P2P_GROUP_REMOVED_EVENT, group);
            P2pStatus p2pStatus = p2pError_GroupRemove(dataString);
            sendMessage(iface, P2P_GROUP_REMOVED_EVENT, sP2pOperFreq, 0, p2pStatus);
            sP2pOperFreq = -1;
            /** @} */
            ...
            P2pStatus p2pStatus = p2pErrorEx(dataString);
            sendMessage(iface, P2P_INVITATION_RESULT_EVENT, sP2pOperFreq, 0, p2pStatus);
            sP2pOperFreq = -1;
            /** @} */
        ...
        ///M:@{
            ...
            /*CTRL-EVENT-DISCONNECTED bssid=02:0c:e7:55:55:55 reason=2
              CTRL-EVENT-DISCONNECTED bssid=02:08:22:04:e8:b6 reason=0 locally_generated=1
              CTRL-EVENT-DISCONNECTED bssid=02:08:22:0e:e2:00 reason=99 locally_generated=1
              peer_oper_freq=2412*/
            String[] tokens = dataString.split(" ");
            ...
            //reconnect channel conflict
            /* CTRL-EVENT-DISCONNECTED bssid=02:08:22:0e:e2:00 reason=99
            locally_generated=1 peer_oper_freq=2412 */
            ...
            /* CTRL-EVENT-DISCONNECTED bssid=02:08:22:0e:e2:00 reason=99 peer_oper_freq=2412 */
            if (tokens.length > 3 && tokens[3].startsWith("peer_oper_freq")) {
                String[] freqNameValue = tokens[3].split("=");
                if (freqNameValue[1] != null) {
                    sP2pOperFreq = Integer.valueOf(freqNameValue[1]);
                    Log.d(TAG, "sP2pOperFreq@CTRL-EVENT-DISCONNECTED-2 = " + sP2pOperFreq);
                }
            }
            //sendMessage(iface, P2P_PEER_DISCONNECT_EVENT, nameValue[1]);
        ...
        if (newState == NetworkInfo.DetailedState.CONNECTED) {
            ...
            if (!match.find()) {
               if (DBG) Log.d(TAG, "handleNetworkStateChange: Could not parse disconnect string");
                ...
                try {
                    ///M: ALPS02711675 Pattern is changed, so index of local should be changed
