...
public class WifiConfigStore {
    ...
    public void readNetworkVariables(WifiConfiguration config) {
        ...
        // M: Add for EAP-FAST begin
        value = mWifiNative.getNetworkVariable(netId, WifiConfiguration.PACFILE_VAR_NAME);
        if (!TextUtils.isEmpty(value)) {
        ...
        value = mWifiNative.getNetworkVariable(netId, WifiConfiguration.PHASE1_VAR_NAME);
        if (!TextUtils.isEmpty(value)) {
            config.phase1 = value;
        } else {
            config.phase1 = null;
        }
        // M: Add for EAP-FAST end
    }
    ...
    private boolean installKeys(WifiEnterpriseConfig existingConfig, WifiEnterpriseConfig config,
        ...
        ///M: modify for WAPI@{
        String privKeyName2 = WifiEnterpriseConfig.USER_PRIVATE_KEY2_PREFIX + name;
        String userCertName2 = Credentials.WAPI_USER_CERTIFICATE + name;
        String caCertName2 = Credentials.WAPI_SERVER_CERTIFICATE + name;
        if (config.getClientCertificate() != null) {
            byte[] privKeyData2 = config.getClientPrivateKey().getEncoded();
             if (isHardwareBackedKey(config.getClientPrivateKey())) {
                ...
                ret = mKeyStore.importKey(privKeyName2, privKeyData2, android.os.Process.WIFI_UID,
                        KeyStore.FLAG_NONE);
            } else {
                ...
                ret = mKeyStore.importKey(privKeyName, privKeyData2, Process.WIFI_UID,
            ...
            if (ret == false) {
                // Remove private key installed
                //mKeyStore.delKey(privKeyName2, Process.WIFI_UID);
                return ret;
            }
        }
        if (config.getCaCertificate() != null) {
             ...
             if (ret == false) {
                 if (config.getClientCertificate() != null) {
                     // Remove client key+cert
                     //mKeyStore.delKey(privKeyName2, Process.WIFI_UID);
                     mKeyStore.delete(userCertName2, Process.WIFI_UID);
                 }
                 return ret;
             }
         }
    ...
    private void removeKeys(WifiEnterpriseConfig config) {
        ...
        if (!TextUtils.isEmpty(client2)) {
            //mKeyStore.delKey(Credentials.WAPI_USER_CERTIFICATE + client2, Process.WIFI_UID);
            mKeyStore.delete(Credentials.WAPI_USER_CERTIFICATE + client2, Process.WIFI_UID);
        }

        String ca2 = config.getCaCertificateWapiAlias();
        // a valid ca2 certificate is configured
        if (!TextUtils.isEmpty(ca2)) {
            mKeyStore.delete(Credentials.WAPI_SERVER_CERTIFICATE + ca2, Process.WIFI_UID);
        }
        ///@}
    }
    ...
    private boolean saveNetwork(WifiConfiguration config, int netId) {
        ...
        if (config.SSID != null && !mWifiNative.setNetworkVariable(
                ...
                encodeSSID(config.SSID, config.isGbkEncoding))) {
    ...
    private boolean updateNetworkKeys(WifiConfiguration config, WifiConfiguration existingConfig) {
        ...
        // M: Add for EAP-FAST begin
        if (config.pacFile != null &&
                !mWifiNative.setNetworkVariable(
                    config.networkId,
                    WifiConfiguration.PACFILE_VAR_NAME,
                    config.pacFile)) {
        ...
        if (config.phase1 != null &&
                !mWifiNative.setNetworkVariable(
                    config.networkId,
                    WifiConfiguration.PHASE1_VAR_NAME,
                    config.phase1)) {
    ...
    public void resetSimNetworks(Collection<WifiConfiguration> configs, int simSlot) {
        if (VDBG) localLog("resetSimNetworks, simSlot: " + simSlot);
        for (WifiConfiguration config : configs) {
            ///M: ALPS02800523 extend to multiple sim,
            //    check config's sim slot is equals to removed sim slot
            if (isSimConfig(config)
                    && WifiConfigurationUtil.getIntSimSlot(config) == simSlot) {
