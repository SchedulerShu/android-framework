...
//      }
    ...
    */
     ...
     */
    private static boolean isImsResourceSupport(Context context, int feature) {
        // Don't care if Dynamic IMS Switch not enabled.
        boolean support = true;
        int mainPhoneId = -1;

        if ("1".equals(SystemProperties.get("persist.mtk_dynamic_ims_switch"))) {
            mainPhoneId = getMainCapabilityPhoneId(context);

            if (!SubscriptionManager.isValidPhoneId(mainPhoneId)) {
                loge("Invalid main phone " + mainPhoneId + ", return true as don't care");
                return support;
            }
            try {
                ImsConfig config = getConfigInterface(mainPhoneId, context);
                if (config != null) {
    ...
    private static boolean isFeatureEnabledByPlatformExt(Context context, int feature) {
        ...
        switch (feature) {
            ...
            case ImsConfig.FeatureConstants.FEATURE_TYPE_VOICE_OVER_WIFI:
                ...
                break;
            default:
                Rlog.w(TAG, "Invalid feature: " + feature);
                break;
        }

        if (context == null) {
            Rlog.w(TAG, "Invalid context, return " + isEnabled);
            return isEnabled;
        }
        IImsManagerExt imsManagerPlugin =
                MPlugin.createInstance(IImsManagerExt.class.getName(), context);
        if (imsManagerPlugin == null) {
            Rlog.w(TAG, "Invalid imsManagerPlugin, return " + isEnabled);
        } else {
            return imsManagerPlugin.isFeatureEnabledByPlatform(feature);
        }
        Rlog.w(TAG, "isFeatureEnabledByPlatformExt(), feature:" + feature +
                    ", isEnabled:" + isEnabled);
